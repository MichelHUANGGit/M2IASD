...
def predict_and_save(model, tokenizer, dataset, device, predict_path, reference_path:str, save_reference=True, temperature=1.0, do_sample=False):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        # Only take the description part of the output, that is everything after '\n Description:\n' except the last EOS token
        predictions[i] = outputs[0][len(inputs['input_ids'][0]):-1].cpu().tolist()
        references[i] = dataset[i]['labels'][:-1]
...
def measure_metrics(predict_path, reference_path):
    ...
    from nlg_eval.metrics import BleuScore, NistScore, MeteorScore, RougeLScore, CiderScore #type: ignore
    bleu = BleuScore()
    nist = NistScore()
    meteor = MeteorScore()
    rouge_l = RougeLScore()
    ...
    scores = dict(
        bleu = bleu.compute(references, predictions),
        nist = nist.compute(references, predictions),
        meteor = meteor.compute(references, predictions),
        rouge_l =  rouge_l.compute(references, predictions),
        cider = cider.compute(references, predictions)
    )
    print(scores)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_path", type=str, help="logs path for the finetuned model")
